...
extends Modelica.Icons.Example;
    ...
    parameter Integer nThColl=28 "Number of thermal collectors in parallel";
  ...
  Buildings.BoundaryConditions.WeatherData.ReaderTMY3 weaDat1(filNam = ModelicaServices.ExternalReferences.loadResource("modelica://RoofKIT/Resources/WeatherFiles/DEU_NW_Dusseldorf.AP.104000_TMYx.2004-2018.mos")) annotation(
    Placement(visible = true, transformation(origin = {-48, 118}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  RoofKIT.Components.Solar.Thermal.ThermalCollector thermalCollector1[nThColl](
    redeclare each final package Medium = Medium_sou, 
    each A_coll = 1.6, 
    each Eta_zero = 0.554, 
    each T_start = 10 + 273.15, 
    each capColl = 0, 
    each final dp_nominal(displayUnit = "Pa") = 50, 
    each final m_flow_nominal = 0.3, 
    each volSol = 0.00154) annotation(
    Placement(visible = true, transformation(origin = {21, -52}, extent = {{-5, 6}, {5, -6}}, rotation = 0)));
 ...
 Buildings.Fluid.MixingVolumes.MixingVolume vol(
   ...
   nPorts = nThColl+1) annotation(
      Placement(visible = true, 
      transformation(origin = {-7, -49}, 
      extent = {{-3, -3}, {3, 3}}, 
      rotation = 0)));
...
// Connections n^th Thermal Collector
 connect(vol.ports[nThColl+1], thermalCollector1[nThColl].port_a) annotation(
    Line(points = {{-6, -52}, {16, -52}}, color = {0, 127, 255}));
 connect(thermalCollector1[nThColl].port_b, STemp_sou1.port_b) annotation(
    Line(points = {{26, -52}, {56, -52}}, color = {0, 127, 255}));
 connect(weaDat1.weaBus, thermalCollector1[nThColl].WeaBusWeaPar);
 
 // Connections Thermal Collectors in loop
  for i in 1:(nThColl-1) loop
    connect(vol.ports[i+1], thermalCollector1[i].port_a);
    connect(thermalCollector1[i].port_b, STemp_sou1.port_b);
    connect(weaDat1.weaBus, thermalCollector1[i].WeaBusWeaPar);
  end for;

//Connections
  connect(weaDat1.weaBus, weaBus) annotation(
    Line(points = {{-38, 118}, {-26, 118}, {-26, 140}, {-194, 140}}, color = {255, 204, 51}, thickness = 0.5));
  connect(weaDat1.weaBus, Gebaeude_modell.weaBus) annotation(
    Line(points = {{-38, 118}, {-25.25, 118}, {-25.25, 132}, {81, 132}}, color = {255, 204, 51}, thickness = 0.5));
...
connect(vol.ports[1], pump_sou.port_b) annotation(
