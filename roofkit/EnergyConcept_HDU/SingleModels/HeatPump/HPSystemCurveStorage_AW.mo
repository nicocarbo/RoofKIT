...
model HPSystemCurveStorage_AW
  ...
  Modelica.Blocks.Sources.Constant mflow_pump1(k = 1) annotation (
  ...
  parameter
    RoofKIT.Database.HeatPump.HeatPump_RoofKIT_AW perHP
  ...
  Modelica.Blocks.Sources.Constant mflowHP(k= 0.8)   annotation (
  ...
  Buildings.Controls.SetPoints.SupplyReturnTemperatureReset watRes(
    ...
    TRet_nominal(displayUnit = "K") = 318.15, TRoo( displayUnit = "K") = 294.15,
    TRoo_nominal(displayUnit = "K") = 295.15,
    TSup_nominal(displayUnit = "K") = 323.15, dTOutHeaBal = 6, m = 1.2,
    use_TRoo_in= false)
  ...
  Modelica.Blocks.Math.UnitConversions.To_kWh energy_sim_kWh annotation (
    Placement(visible = true, transformation(extent = {{152, -122}, {172, -102}}, rotation = 0)));
  Modelica.Blocks.Continuous.Integrator energy_HP(k = 1) annotation (
    Placement(visible = true, transformation(extent = {{114, -122}, {134, -102}}, rotation = 0)));
protected
...
equation
  ...
  connect(heaPum.P, energy_HP.u) annotation (
    Line(points = {{94, -48}, {103, -48}, {103, -112}, {112, -112}}, color = {0, 0, 127}));
  connect(energy_HP.y, energy_sim_kWh.u) annotation (
    Line(points = {{136, -112}, {150, -112}}, color = {0, 0, 127}));
  annotation (
