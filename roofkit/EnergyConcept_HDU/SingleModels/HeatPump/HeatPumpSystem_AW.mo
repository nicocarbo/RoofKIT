...
model HeatPumpSystem_AW "Example for a heat pump system with the ISO 13790 building model"
  ...
  Buildings.Fluid.Sources.MassFlowSource_T sou(redeclare package Medium = Medium_sou, T = 283.15, m_flow = 0.25, nPorts = 1) "Fluid source on source side" annotation (
  ...
  parameter RoofKIT.Database.HeatPump.HeatPump_RoofKIT_AW per "Reverse heat pump performance data" annotation (
  ...
  Modelica.Blocks.Sources.Constant TSetHP(k = 273.15 + 35) annotation (
    Placement(visible = true, transformation(origin = {-74, -46}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Sources.Constant mflowHP(k = 0.3) annotation (
  ...
  Modelica.Blocks.Math.UnitConversions.To_kWh energy_sim_kWh annotation (
    Placement(visible = true, transformation(extent = {{122, -108}, {142, -88}}, rotation = 0)));
  Modelica.Blocks.Continuous.Integrator energy_HP(k = 1) annotation (
    Placement(visible = true, transformation(extent = {{84, -108}, {104, -88}}, rotation = 0)));
equation
...
//  experiment(StopTime=31536000, Interval=3600, Tolerance=1e-06, __Dymola_Algorithm="Dassl"),
  ...
  connect(energy_sim_kWh.u, energy_HP.y) annotation (
    Line(points = {{120, -98}, {106, -98}}, color = {0, 0, 127}));
  connect(energy_HP.u, heaPum.P) annotation (
    Line(points = {{82, -98}, {28, -98}, {28, -96}}, color = {0, 0, 127}));
  annotation (
