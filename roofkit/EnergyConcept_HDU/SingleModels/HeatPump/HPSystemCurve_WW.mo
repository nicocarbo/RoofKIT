...
model HPSystemCurve_WW "Example for a heat pump system with the ISO 13790 building model"
  ...
  Buildings.Fluid.Sources.Boundary_pT preSou(redeclare package Medium = Medium_sin, T = 308.15, nPorts = 1) "Source for pressure and to account for thermal expansion of water" annotation (
    Placement(transformation(extent = {{-78, -20}, {-58, 0}})));
  Buildings.Fluid.Sources.MassFlowSource_T sou(redeclare package Medium = Medium_sou, T = 283.15, m_flow = 0.4, nPorts = 1) "Fluid source on source side" annotation (
    Placement(transformation(extent = {{68, -138}, {48, -118}})));
  Buildings.Fluid.Sources.Boundary_pT sin(redeclare package Medium = Medium_sou, T = 279.15, nPorts = 1) "Fluid sink on source side" annotation (
    Placement(transformation(extent = {{-78, -138}, {-58, -118}})));
  Buildings.Fluid.Sensors.TemperatureTwoPort senT_a1(redeclare final package Medium = Medium_sin, allowFlowReversal = true, m_flow_nominal = 1, transferHeat = true) "Temperature at sink inlet" annotation (
  ...
  Buildings.Fluid.HeatPumps.EquationFitReversible heaPum(redeclare package Medium1 = Medium_sin, redeclare
      package                                                                                                      Medium2 = Medium_sou,  energyDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, massDynamics = Modelica.Fluid.Types.Dynamics.FixedInitial, T1_start = 301.4, T2_start =  283.15, per = per) "Water to Water heat pump" annotation (
  ...
  parameter RoofKIT.Database.HeatPump.HeatPump_RoofKIT_WW per "Reverse heat pump performance data" annotation (
    Placement(transformation(extent = {{106, -26}, {126, -6}})));
  Buildings.Fluid.Movers.FlowControlled_m_flow pumHeaPum(redeclare replaceable
      package                                                                          Medium = Medium_sin, energyDynamics = Modelica.Fluid.Types.Dynamics.SteadyState, m_flow_nominal = 1, nominalValuesDefineDefaultPressureCurve = true, use_inputFilter = false) annotation (
  ...
  Buildings.Controls.SetPoints.SupplyReturnTemperatureReset watRes(TOut_nominal(displayUnit = "K") = 265.15, TRet_nominal(displayUnit = "K") = 308.15, TRoo(displayUnit = "K") = 294.15, TRoo_nominal(displayUnit = "K") = 294.15, TSup_nominal(displayUnit = "K") = 313.15, dTOutHeaBal = 6, m = 1.2, use_TRoo_in = false) annotation (
    Placement(visible = true, transformation(extent = {{-126, -56}, {-106, -36}}, rotation = 0)));
  Buildings.BoundaryConditions.WeatherData.Bus weaBus annotation (
    Placement(visible = true, transformation(extent = {{106, 54}, {126, 74}}, rotation = 0), iconTransformation(extent = {{148, 50}, {168, 70}}, rotation = 0)));
  Modelica.Blocks.Continuous.Integrator energy_HP(k = 1) annotation (
    Placement(visible = true, transformation(extent = {{86, -116}, {106, -96}}, rotation = 0)));
...
equation
  ...
  connect(rad.heatPortRad, Gebaeude_modell.port_surf) annotation (
    Line(points = {{-2, -2.8}, {-2, 60}, {52, 60}, {52, 102}}, color = {191, 0, 0}));
  connect(rad.heatPortCon, Gebaeude_modell.port_air) annotation (
    Line(points = {{2, -2.8}, {2, 64}, {48, 64}, {48, 106}, {52, 106}}, color = {191, 0, 0}));
  ...
  connect(movMea.y, heaPum.TSet) annotation (
    Line(points={{-70,-44},{-29.64,-44},{-29.64,-72.5}},color = {0, 0, 127}));
  connect(watRes.TSup, movMea.u) annotation (
    Line(points={{-105,-40},{-94,-40},{-94,-44}},        color = {0, 0, 127}));
  connect(weaDat1.weaBus, weaBus) annotation (
    Line(points = {{120, 130}, {116, 130}, {116, 64}}, color = {255, 204, 51}, thickness = 0.5));
  connect(watRes.TOut, weaBus.TDryBul) annotation (
    Line(points = {{-128, -40}, {-142, -40}, {-142, -26}, {38, -26}, {38, 20}, {116, 20}, {116, 64}}, color = {0, 0, 127}));
  connect(energy_sim_kWh.u, energy_HP.y) annotation (
    Line(points = {{118, -106}, {108, -106}}, color = {0, 0, 127}));
  connect(energy_HP.u, heaPum.P) annotation (
    Line(points = {{84, -106}, {68, -106}, {68, -96}, {28, -96}}, color = {0, 0, 127}));
  annotation (
    Diagram(coordinateSystem(preserveAspectRatio = false, extent = {{-160, -160}, {160, 160}}), graphics={  Rectangle(origin = {4, 104}, fillColor = {170, 170, 127},
            fillPattern =                                                                                                                                                           FillPattern.Solid, extent = {{-154, 54}, {154, -54}}), Text(origin = {-13, 136}, extent = {{-53, 22}, {53, -22}}, textString = "Gebäude und Wetter"), Rectangle(origin = {2, -57}, fillColor = {255, 147, 147},
            fillPattern =                                                                                                                                                                                                        FillPattern.Solid, extent = {{156, -97}, {-156, 97}}), Text(origin = {-113, 18}, extent = {{-39, 34}, {39, -34}}, textString = "Wärmepumpe
Heizkörper
Vorlauftemp 40°C")}),
